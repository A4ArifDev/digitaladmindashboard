...
const SideBar = () => {
  ...
  const menus = [
    ...
    {name:"blogs", link:"/blogs", icon:TfiLayoutSliderAlt},
    {name:"gigs", link:"/gigs", icon:AiOutlinePicture},
    {name:"users", link:"/users", icon:AiOutlineUsergroupAdd},
    {name:"messages", link:"/messages", icon:BiMessageRounded},
  ]
  ...
  return (
    <>
        <div className={`${open ? "basis-[250px]" : "basis-[66px] md:basis-[18%]"} bg-subBgLight py-[30px] dark:bg-sideBarBgDark h-full overflow-hidden transition-al flex justify-between flex-col`}>
                  ...
                  <ul className='mt-[40px]'>
                      {menus.map((menu, i)=>{
                        return(
                          <li key={i}>
                            <NavLink className={({ isActive }) => (isActive ? 'active-nav-links' : 'nav-links')} to={menu.link}>{React.createElement(menu.icon, {className:'text-[#698EFF] text-2xl flex-none'})}<span className={`text-sm font-medium capitalize flex-none ${!open && "hidden md:block"}`}>{menu.name}</span></NavLink>
                          </li>
                        )
                      })}
                  </ul>
                  <ul className='mt-[30px] px-1 md:px-4'>
